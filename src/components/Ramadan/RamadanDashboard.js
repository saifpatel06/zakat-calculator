...
const RamadanDashboard = ({ initialTimings, initialHijri }) => {

  const [mounted, setMounted] = useState(false);
  const [prayerTimes, setPrayerTimes] = useState(initialTimings);
  const [hijriDate, setHijriDate] = useState(initialHijri);
  const [ramadanDay, setRamadanDay] = useState(
    initialHijri?.monthNumber === 9
      ? parseInt(initialHijri.day)
      : null
  );

  const [cityLabel] = useState("Pune, India");
  ...
  useEffect(() => {
    ...
    setDeed(deeds[dayOfMonth % deeds.length]);
  }, []);
  ...
  const calculateTimeToNextPrayer = () => {
    ...
    for (let prayer of prayers) {
      ...
      if (prayerMinutes > currentMinutes) {
        ...
        setTimeToNextPrayer(`${Math.floor(diff / 60)}h ${diff % 60}m`);
        return;
      }
    }
          ...
          <div className={styles.heroContent}>
            ...
            <div className={styles.timeDateCard}>
              <div className={styles.liveTime}>
                {mounted ? formatCurrentTime() : "--:--:--"}
              </div>
              <div className={styles.gregorianFull}>
                {formatGregorianDate()}
              </div>
          ...
          {ramadanDay && (
            <div className={styles.ramadanBanner}>
              <div className={styles.ramadanText}>
                ☪️ Ramadan Day {ramadanDay} of 30
              </div>
